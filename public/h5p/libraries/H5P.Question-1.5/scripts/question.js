...
H5P.Question = (function ($, EventDispatcher, JoubelUI) {
  ...
  // Add methods that should be inherited from H5P.ContentType
  self.isRoot = function () {
    return (self.parent === undefined);
  };

  self.setActivityStarted = function () {
    if (H5P.xAPIEvent) {
      const xAPIEvent = new H5P.XAPIEvent();
      xAPIEvent.setActor();
      xAPIEvent.setVerb('experienced');
      xAPIEvent.setObject(self);
      xAPIEvent.setContext(self.parent);
      xAPIEvent.setResult('0', '0', '', false);
      self.trigger(xAPIEvent);
    }
  };

  self.triggerXAPIScored = function (score, maxScore, verb) {
    if (H5P.xAPIEvent) {
      const xAPIEvent = new H5P.XAPIEvent();
      xAPIEvent.setActor();
      xAPIEvent.setVerb(verb || 'scored');
      xAPIEvent.setObject(self);
      xAPIEvent.setContext(self.parent);
      xAPIEvent.setResult(score, maxScore, '', score === maxScore);
      self.trigger(xAPIEvent);
    }
  };
